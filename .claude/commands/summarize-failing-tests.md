...
allowed-tools: Bash(npm test:*), Bash(npm --prefix:*), Bash(npx vitest:*), Read, Grep, Glob
---
...
1. Run the tests, capturing output. The runnable app/tests live in `examples/demo-app` (vitest):
   `npm --prefix examples/demo-app test`
   - If `$ARGUMENTS` is non-empty, treat it as a path/name filter:
     `npx --prefix examples/demo-app vitest run $ARGUMENTS`
   - If the repo grows other test suites later, run each and merge the failures.
